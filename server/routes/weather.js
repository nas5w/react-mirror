...
module.exports = (req, res) => {
  ...
  if (req.body.zip.length !== 5) {
    return res
      .status(400)
      .send({ message: `Zip code ${req.body.zip} is incorrectly formatted.` });
  }
  ...
  const forecast_uri = `${base_uri}/forecast?zip=${
    req.body.zip
  },us&units=imperial&appid=${weather_api_key}`;

  return Promise.all([rp(current_uri), rp(forecast_uri)]).then(response => {
